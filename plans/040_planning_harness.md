# Plan de Construcción — 040 Planning Harness

## Meta

Construir el harness completo para la fase Planning implementando el Patrón Universal de Fase
de `Insumos/metodologia.md`. Este plan es el blueprint que guía la construcción del harness
operativo completo.

El 040 toma el draft de Vertical Slices producido por el 030 en `030_design/test_strategy_map.md`
y lo convierte en el **plan maestro completo** del proyecto: slices formalmente definidas con
scope, Done y esfuerzo; roadmap con dependencias y hitos; y registro de riesgos por slice.
Es el último harness de la Capa 1 — Gobernanza. Su output es la fuente de verdad que el
050 Vertical Harness consume slice a slice.
...
1. **Inventario y validación del draft VS** — Leer I-1 y extraer la lista completa de
   ...
   se documentan pero no se fusionan (el mínimo aplica a la estructura de slices, no
   a eliminar slices existentes).
...
4. **Roadmap de dependencias y secuencia** — Usando I-5 (DEP-xx), derivar las
   ...
   obligatoria: Tracer Bullet siempre primero → Crecimiento (en cualquier orden
   entre sí, respetando deps) → MVP → Evolución (ídem) → Robustez siempre último.
   Identificar hitos: Tracer Bullet, MVP y Robustez son hitos obligatorios.

5. **Registro de riesgos por slice** — Para cada VS-xx, identificar ≥1 riesgo
   (RK-xx) relevante: complejidad técnica, dependencias externas, ambigüedades no
   resueltas en scope, decisiones de arquitectura que podrían requerir revisión.
   Asignar probabilidad (Alta/Media/Baja) e impacto (Alto/Medio/Bajo) y proponer
   una mitigación concreta.
...
Artefacto intermedio (no entregado al 050):
...
1. Todas las VS-xx del draft del 030 han sido validadas: slices sobredimensionadas divididas,
   slices dentro del límite conservadas tal cual.
...
| B — Orchestrator | `planning-orchestrator` | Capataz Técnico | Lee contrato; escribe Demo Statements en orchestration_plan; persiste plan antes de spawear; registra checkpoints; verifica artefactos en disco (Pending Verification) | `persistence/execution-state.json` |
...
- **A spawnea Workers directamente.** (ver nota)
...
- El governor spawnea los Workers directamente (planning-analyst, planning-writer).
- El orchestrator opera en modos 040_planning/CHECKPOINT: persiste el estado pero no spawnea Workers.
...
|--------|-------------|-------------------|---------------|
| `planning-analyst` | Lee los 12 inputs (I-1..I-12). Valida granularidad del draft VS, detecta IC-xx y BDD scenarios huérfanos, extrae dependencias entre slices e identifica riesgos por slice. Produce planning_analysis_report.md | Paths a I-1..I-12 | `/040_planning/planning_analysis_report.md` |
| `planning-writer` | Lee planning_analysis_report.md + inputs de referencia. Produce los 3 artefactos finales en orden. | Path a planning_analysis_report.md + paths a I-1, I-4, I-6, I-12 | `/040_planning/vertical_slice_plan.md`, `/040_planning/project_roadmap.md`, `/040_planning/risk_register.md` |
...
antes de que el governor spawnee ningún Worker. Formato:
...
> `project_roadmap.md` listas todas las VS-xx en secuencia respetando la estructura
...
| CP-03 | Cliente revisa draft | A presenta los 3 artefactos al cliente; registra feedback en `harness-state.json` |
...
> requerida para evaluar umbrales). Hito MVP marcado en VS-03. Duración estimada: M (VS-03 = L,
> otros 2-3 bloques de integración). `risk_register.md`: RK-03 para VS-03 — "Dependencia de servicio
> SMTP externo para IC-06. Probabilidad: Media. Impacto: Alto (bloquea el MVP). Mitigación: usar
> stub de INotificacionService en el Tracer Bullet; configurar SMTP real solo en VS-03; tener
> proveedor alternativo documentado en ADR-002." Sin ningún ID referenciado en el plan que no
> exista en contract_definitions.md o bdd_features.md.
...
## Sección 5 — Handoff Artifact → 050 Vertical Harness

El 040 entrega al 050 los siguientes artefactos. El 050 no puede iniciarse sin ellos.
...
Verificar que `persistence/harness-state.json` existe y que la clave `"030_design"` tiene `"status": "PHASE_COMPLETE"`.

Si no existe o el status del 030 es distinto de `"PHASE_COMPLETE"`: **detener flujo**. Notificar al humano:
> "El 030 Design debe completarse antes de iniciar el 040 Planning. Estado actual: [valor encontrado]."

**Determinación del modo:**
- No existe clave `"040_planning"` en `harness-state.json` → **Modo INICIO** → ejecutar Ritual E10-A
- Existe clave `"040_planning"` e íntegra → **Modo CONTINUACIÓN** → ejecutar Ritual E10-B
...
3. Leer `persistence/harness-state.json` completo. Agregar clave `"040_planning"` con status
   `"PENDING_CONTRACT"` sin modificar ninguna clave existente (raíz ni `"030_design"`).
...
- A presenta los 3 artefactos de `/040_planning/` al cliente para revisión (CP-03)
- **IMPORTANTE (ADJ-23):** Registrar `[CP-03 040]` en `claude-progress.txt` con `Add-Content -Encoding utf8`
  ANTES de presentar los artefactos. Aunque el cliente incluya aprobación en la misma respuesta del
  CP-03, presentar CP-04 como `AskUserQuestion` separado e independiente.
...
- A escribe `"040_planning.status": "AUDIT_PENDING"` en `harness-state.json`
- A registra `[AUDIT_PENDING 040]` en `claude-progress.txt`
- A spawea planning-evaluator pasando paths a los 3 artefactos + paths de referencia I-4, I-6, I-12

**Paso 3 — Auditoría (C — planning-evaluator):**
- C lee los 3 artefactos desde el filesystem (sin contexto de ejecución)
- C lee `030_design/contract_definitions.md`, `020_specification/bdd_features.md`,
  `010_discovery/domain_glossary.md` como referencia para D1, D3 y D5 (verificación independiente)
- C evalúa contra rúbrica (Sección 4), aplica anclas de calibración — dos fases: análisis con citas → score (LL-07)
- C escribe (PATHS DE SALIDA — OBLIGATORIO: solo en `eval/`, nunca en `/040_planning/`):
...
- A marca `"040_planning.status": "IN_REWORK"` en `harness-state.json`
- A spawea planning-orchestrator en modo PLAN pasando referencia al rechazo
- Orchestrator escribe nuevo plan; A spawea solo el Worker que produce el artefacto fallido
...
- A detiene flujo, marca `"040_planning.status": "HOLD"` en `harness-state.json`
...
2. A actualiza `knowledge/lessons_learned.md` con hallazgos del ciclo
...
4. A notifica al humano: lista de 3 artefactos producidos, paths, VS-xx totales planificadas,
   hitos del proyecto (Tracer Bullet, MVP, Robustez)
...
- Sí → A registra `"handoff_050": {"status": "DEPLOYED"}`, ejecuta deploy via
  `$env:HARNESS_DEPLOY_SCRIPT 050`, instruye al humano: **"Reinicia la sesión. El CLAUDE.md
  detectará el estado DEPLOYED y arrancará planning-governor automáticamente."**
...
### Agentes a crear (todos nuevos, sin herencia del 030)

```
.claude/agents/
├── planning-governor.md      — Instancia A (modos INIT/EXECUTE/POST_CP03/POST_CP04/CLOSE)
├── planning-orchestrator.md  — Instancia B (modos 040_planning/CHECKPOINT/WORKER_FAILED — sin Agent tool)
├── planning-analyst.md       — Worker 1 (tools: Read, Write)
├── planning-writer.md        — Worker 2 (tools: Read, Write, Edit)
├── planning-reviewer.md      — Control pre-CP-03 (tools: Read, Write)
└── planning-evaluator.md     — Instancia C (tools: Read, Write — solo escribe en eval/)
```
...
```
.claude/skills/
├── planning-state-schema.md       — Schema de entrada "040_planning" en harness-state.json + execution-state.json
├── planning-analysis-schema.md    — Schema de planning_analysis_report.md (secciones obligatorias)
├── planning-analyst-protocol.md   — Pasos de extracción: validación granularidad, asignación, deps, riesgos
├── planning-synthesis-schema.md   — Schema de los 3 artefactos finales (VS-xx 6 campos, RK-xx, roadmap)
├── planning-writer-protocol.md    — Reglas de transformación analysis→artefactos + checklist cruzado
├── planning-rubric.md             — Rúbrica D1-D5 con anclas 0.2/0.5/0.8/1.0 y few-shot calibrado
├── planning-verdict-schema.md     — verdict.json + metrics_summary.json (array acumulativo, phase: "040_planning")
└── planning-evaluator-protocol.md — Verificación D1-D5 + cross-checks de IDs entre los 3 artefactos
```
...
|---------|-----------------|
| `templates/client-project-CLAUDE.md` | Agregar condición: si 030 PHASE_COMPLETE y 040 no → invocar planning-governor |
| `deploy-harness.ps1` | Agregar soporte para harness `040` en la lógica de selección |
| `.claude/agents/design-governor.md` | Handoff al 040: verificar que usa `$env:HARNESS_DEPLOY_SCRIPT 040` correctamente |

### IDs de referencia cruzada

| Artefacto | ID | Formato | Ejemplo |
|-----------|-----|---------|---------|
| planning_analysis_report | Slice analizada | VS-xx | VS-03 |
| planning_analysis_report | Riesgo preliminar | RK-xx (provisional) | RK-02 |
| vertical_slice_plan | Vertical Slice | VS-xx | VS-03 |
| project_roadmap | Referencia a slice | VS-xx | VS-03 |
| risk_register | Riesgo | RK-xx | RK-05 |

Los VS-xx son los mismos que los definidos en `030_design/test_strategy_map.md` del 030.
Si se dividen slices sobredimensionadas, los IDs nuevos siguen la secuencia del último
VS-xx existente (ej: si el draft tiene VS-01..VS-05 y VS-03 se divide, las nuevas son
VS-03a/VS-03b o VS-06/VS-07 — el analyst decide y documenta la convención elegida).

### Lecciones críticas a aplicar (LL-01 a LL-28)

Las 28 lecciones aplican íntegramente. Las más críticas para el 040:

| Lección | Aplicación en 040 |
|---------|------------------|
| LL-01 | Workers escriben artefacto ANTES de reportar COMPLETED |
| LL-02 | planning-orchestrator tiene sección REGLAS DE ESCRITURA al inicio |
| LL-03 | planning-evaluator tiene bloque PATHS DE SALIDA — escribir solo en `eval/` |
| LL-04 + LL-20 | Cierre del governor: verificación de disco como primer tool call (no texto) |
...
| LL-10 | planning-governor NUNCA escribe en `/040_planning/` |
| LL-13 | Orchestrator verifica artefacto en disco antes de registrar checkpoint (Pending Verification ADJ-13) |
| LL-17 | Si la rúbrica verifica alguna frase textual en un artefacto, el schema del writer debe especificarla exacta |
| LL-20 | Cierre: verificación de eval/verdict.json como PRIMER tool call del Cierre |
| LL-21 | planning-orchestrator SIN Agent tool; governor spawnea workers directamente |
| LL-22 | Handoff instruye reinicio de sesión, no spawn directo |
| LL-24 | Demo Statements citan explícitamente las secciones obligatorias de cada artefacto |
| LL-25 | CP-03 y CP-04 siempre como AskUserQuestion separadas |
| LL-27 | planning-reviewer entre CP-02 y CP-03 (verifica IDs cruzados entre los 3 artefactos) |
