...
Construir el harness completo para la fase Design implementando el Patrón Universal de Fase
de `Insumos/metodologia.md`. La base conceptual ya existe en `Harnesses/030_design_harness.md`
(definición de alto nivel). Este plan es el blueprint que guía la construcción del harness
operativo completo.

El 030 transforma los contratos formales del 020 (QUÉ hace el sistema) en el plano
técnico-arquitectónico (CÓMO está construido). Es el primer harness de la Capa 2 — Ejecución.
...
7. **Guía de Vertical Slices (ADJ-04)** — Identificar cómo el sistema puede ser sliceado
   en iteraciones verticales funcionales de extremo a extremo. Esto no es el planning (eso
   es el 040) — es identificar las fronteras naturales de slicing para que el 040 pueda
   planificar iteraciones coherentes con la arquitectura propuesta.
...
| B — Orchestrator | `design-orchestrator` | Capataz Técnico | Lee contrato; escribe Demo Statements en orchestration_plan; persiste plan antes de spawear; registra checkpoints; verifica artefactos en disco (Pending Verification) | `persistence/execution-state.json` |
...
- **A NO llama Workers directamente.** (ver nota)
...
### Demo Statements (ADJ-13 — obligatorio en el 030)
...
> (del test_specification_003). Almacenista, jefa de compras, gerente como actores.
...
## Sección 5 — Handoff Artifact → 040 Planning
...
Verificar que `persistence/harness-state.json` existe y que la clave `"020_specification"` tiene `"status": "PHASE_COMPLETE"`.

Si no existe o el status del 020 es distinto de `"PHASE_COMPLETE"`: **detener flujo**. Notificar al humano:
> "El 020 Specification debe completarse antes de iniciar el 030 Design. Estado actual: [valor encontrado]."

**Determinación del modo:**
- No existe clave `"030_design"` en `harness-state.json` → **Modo INICIO** → ejecutar Ritual E10-A
- Existe clave `"030_design"` e íntegra → **Modo CONTINUACIÓN** → ejecutar Ritual E10-B
...
- A marca `"030_design.status": "IN_REWORK"` en `harness-state.json`
- A spawea design-orchestrator en modo PLAN pasando referencia al rechazo
...
- A detiene flujo, marca `"030_design.status": "HOLD"` en `harness-state.json`
...
4. A notifica al humano: lista de 5 artefactos producidos, paths, estado listo para 040
...
- Sí → A registra `"handoff_040": {"status": "DEPLOYED"}`, ejecuta deploy via
  `$env:HARNESS_DEPLOY_SCRIPT 040`, instruye al humano: **"Reinicia la sesión. El CLAUDE.md
  detectará el estado DEPLOYED y arrancará design-governor automáticamente."**
...
### Agentes a crear (todos nuevos, sin herencia del 020)
...
```
.claude/skills/
├── design-state-schema/       — Schema de entrada "030_design" en harness-state.json + execution-state.json
├── design-analysis-schema/    — Schema de design_analysis_report.md (IDs: CO-xx, IF-xx, PT-xx, RT-xx)
├── design-analyst-protocol/   — Categorías de extracción del analyst
├── design-synthesis-schema/   — Schema de los 5 artefactos finales (IDs: MOD-xx, IC-xx, DEP-xx, ADR-xx, TS-xx)
├── design-architect-protocol/ — Reglas de transformación analysis→artefactos + checklist consistencia cruzada
├── design-rubric/             — Rúbrica D1-D5 con anclas 0.2/0.5/0.8/1.0 y few-shot calibrado
├── design-verdict-schema/     — verdict.json + metrics_summary.json (array acumulativo, phase: "030_design")
└── design-evaluator-protocol/ — Verificación D1-D5 + cross-checks de IDs
```
...
|---------|-----------------|
| `templates/client-project-CLAUDE.md` | Agregar condición: si 020 PHASE_COMPLETE y 030 no → invocar design-governor |
| `deploy-harness.ps1` | Agregar soporte para harness `030` en la lógica de selección |
| `.claude/agents/specification-governor.md` | Handoff al 030: asegurar que la instrucción usa `subagent_type: 'design-governor'` |

### IDs de referencia cruzada

| Artefacto | ID | Formato | Ejemplo |
|-----------|-----|---------|---------|
| design_analysis_report | Componente | CO-xx | CO-01 |
| design_analysis_report | Interface requerida | IF-xx | IF-03 |
| design_analysis_report | Patrón propuesto | PT-xx | PT-02 |
| design_analysis_report | Restricción tecnológica | RT-xx | RT-01 |
| technical_blueprint | Módulo | MOD-xx | MOD-04 |
| contract_definitions | Interface Contract | IC-xx | IC-02 |
| contract_definitions | DTO | DTO-xx | DTO-07 |
| dependency_graph | Dependencia | DEP-xx | DEP-03 |
| architecture_decision_records | ADR | ADR-xx | ADR-001 |
| test_strategy_map | Test Strategy item | TS-xx | TS-05 |

### Lecciones críticas a aplicar (LL-01 a LL-23)

Las 23 lecciones aplican íntegramente. Las más críticas para el 030:

| Lección | Aplicación en 030 |
|---------|------------------|
| LL-01 | Workers escriben artefacto ANTES de reportar COMPLETED |
| LL-02 | design-orchestrator tiene sección REGLAS DE ESCRITURA al inicio |
| LL-03 | design-evaluator tiene bloque PATHS DE SALIDA — escribir solo en `eval/` |
...
| LL-21 | design-orchestrator SIN Agent tool; governor spawea workers directamente |
