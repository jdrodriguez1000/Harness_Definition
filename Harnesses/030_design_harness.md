...
| Test Strategy Map | `/design/test_strategy_map.md` | Qué se probará con unitarios/integración/contrato por interface; puntos de mock/stub; Guía de Vertical Slices (TB → Crecimiento 0..N → MVP → Evolución 0..M → Robustez) con 5 campos por slice |
...
- `/design/review_report.md` — producido por design-reviewer entre CP-02 y CP-03; verifica consistencia estructural pre-aprobación
...
- D NO llama a nadie. Solo lee del filesystem y escribe `design/review_report.md`.
...
|--------|-------------|-------------------|---------------|
| `design-analyst` | Lee los 8 inputs (I-1..I-8). Identifica bounded contexts, interfaces requeridas, patrones aplicables, restricciones tecnológicas. Produce design_analysis_report.md. | Paths a I-1..I-8 + Demo Statement del orchestration_plan | `/design/design_analysis_report.md` |
| `design-architect` | Lee design_analysis_report.md + domain_glossary.md + scope_boundaries.md. Selecciona stack, produce los 5 artefactos finales en orden. | Path a design_analysis_report.md + paths a I-2, I-6, I-7 + Demo Statement | `/design/technical_blueprint.md`, `/design/contract_definitions.md`, `/design/dependency_graph.md`, `/design/architecture_decision_records.md`, `/design/test_strategy_map.md` |
...
| — | Tras CP-02 (pre-CP-03) | A spawea design-reviewer. Si issues críticos → rework. Reviewer produce `design/review_report.md` |
...
2. A presenta los 5 artefactos de `/design/` al cliente para revisión (CP-03)
...
2. C lee `specification/bdd_features.md`, `specification/data_contracts.md`,
...
5. C escribe (**PATHS DE SALIDA — OBLIGATORIO: solo en `eval/`, nunca en `/design/`** — LL-03):
...
4. A notifica al humano con resumen de cierre:
   - Artefactos producidos y sus paths (los 5 artefactos en `/design/`)
